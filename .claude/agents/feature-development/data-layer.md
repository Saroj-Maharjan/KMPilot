...
**Data Patterns:** @../../skills/create-feature/architecture/data.md (load on demand)
**Local Persistence:** @../../skills/create-feature/architecture/local-data.md — **reference only**. You implement **remote** feature data layers. On-device persistence (theme/locale/token, DataStore/Room) is core infra in `:core:data`, hand-authored — do NOT generate it. If a feature genuinely owns persisted state, follow this doc's pattern (no `Either`; `*LocalDataSource` + `*Repository` over a shared backend).
**Gradle Template:** @../../skills/create-feature/architecture/build-gradle-template.md (use when scaffolding `feature/{featurename}/build.gradle.kts`)
