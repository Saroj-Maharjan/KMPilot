...
3. **Grow the theme after approval**: Run a Color Audit (Phase 1 Step 1.8). Every missing M3 role must be added to **both** `XLightColors` and `XDarkColors` in `XTheme.kt` BEFORE any feature code is written.
