---
description: Modify existing KMP features with spec-first workflow. Invoke with /modify-feature.
...
- Data changes: @../create-feature/architecture/data.md
- UI changes: @../create-feature/architecture/ui.md
- **Platform capability / native-view changes (Rule 14)**: @../create-feature/architecture/platform.md
- Integration changes: @../create-feature/architecture/integration.md
- Bottom-bar tab changes ("add/remove bottom-bar tab", "make this a tab", "show in bottom nav"): @../create-feature/architecture/integration.md → "5. Bottom-Bar Tab (Optional)"
...
**Platform capability / native view (Rule 14)**: when the change adds a device capability or native view, follow `@../create-feature/architecture/platform.md`:
...
2. **Generate a `@Preview` composable** in the same file as the new component, marked `private`, wrapped in `XTheme`, with realistic sample data. Use the canonical import `androidx.compose.ui.tooling.preview.Preview` (CMP 1.11.0+ — common). Never use the deprecated `org.jetbrains.compose.ui.tooling.preview.Preview`.
   ...
   See `@../create-feature/architecture/ui.md` → "Previews" for the full pattern, including `@PreviewParameter` for multi-variant previews.
...
3. **Component implementation** — Implement UI from the blueprint's Component Tree. Use the blueprint as the primary source, design screenshots as visual cross-reference only. Apply constraint resolutions from sub-step 2. Every text node uses `style = MaterialTheme.typography.{role}` (or an `XTextDefaults` preset) — never raw `fontSize`/`fontWeight`, except a *Type-scale role override* row (`…typography.{role}.copy(...)`). Never set `fontFamily` (global, wired in 1b). **Motion** (if the blueprint has a `## Motion` table): write feature-specific rows into `presentation/ui/motion/{Feature}Motion.kt`, call the DS `motion/` primitives (from 1c) for generic rows, gate each with `rememberReducedMotion()` — never inline, never interaction/hover motion.
   - **Images** follow each `images.json` entry's `delivery`: `bundled` → `Image(painter = painterResource({res_reference}))` (materialize the raster first if absent: run `python3 .claude/skills/_shared/download_assets.py --type images --feature {featurename} --project-root {repo_root} --html …` **without** `--manifest-only` — it downloads `bundled` only and skips `remote`); `remote` → `AsyncImage(url = {data_binding}, loadingResId = DesignSystemResources.drawable.ds_image_placeholder, …)` (design-system `AsyncImage`, `url=`) bound to a data-layer field, **never** the Stitch CDN URL — add/wire that field on the DTO/`*UiModel` and keep its Post-Implementation Checklist item. When materializing icons/images, pass **one `--html .../stitch_{role}.html` per `secondaryScreens[]` entry** alongside the selected-state HTML (secondary screens may add their own assets).
3b. **Secondary screens** (only when `features[{featurename}].secondaryScreens[]` is non-empty) — implement the blueprint's `Component Tree → Secondary Screens` subsections per `kind` (see [`_shared/patterns.md` → "Secondary Screens within a Feature"](../_shared/patterns.md)): **`surface`** → render the role's host X-component (`XModalBottomSheet`/`XDialog`/`XAlertDialog`/`XModalDrawerSheet`/inline `XSurface`) from `{Feature}ScreenRoot`, gated by a `{Feature}UiModel` visibility field, opened/closed via callbacks (Rule 10) — no Route; **`screen`** → a full child screen with its own `{Feature}{Role}Screen.kt` (own allowlist) + `components/` + `{Feature}{Role}Route` + a `composable<{Feature}{Role}Route>` inside the same `NavGraphBuilder.{featurename}()` extension, navigated from the primary via a hoisted callback. A child route of the feature — **not** a new Integration Point.
...
Build errors: Load @../create-feature/troubleshooting/index.md
...
> **Next step —** run `/clear` to free the context window (the spec + design blueprint are durable artifacts — the next skill re-reads them fresh, so clearing loses nothing), then `/review-feature {featurename}` to validate the changes — or `/verify-ui {featurename}` if you applied a design.
