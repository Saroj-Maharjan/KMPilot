...
Before writing the `XNavigationBar`/`XNavigationBarItem` call in `App.kt`, read the feature's blueprint `### Component Overrides` table for any rows referencing `XNavigationBar` or `XNavigationBarItem` (e.g. `shape`, `containerColor`, `colors`, `modifier`, `elevation`). Apply every such override in the generated `App.kt` code — they cover app-shell chrome wired at Point 5, not only feature-internal components. This is not a hardcoded property list; it is driven by what `/design-ui` recorded.
