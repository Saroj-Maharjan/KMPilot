...
| 3 | **iOS-Swift bridge** (`/bridge-swift`) | the iOS actual needs Swift (SDK not clean from Kotlin/Native) | highest — Swift + DI plumbing |
...
2. Leave the Swift impl unwritten and **surface a follow-up**:
   ...
   > Run `/bridge-swift` for `{Feature}Bridge` to implement the Swift side, then continue.

Skills never call each other — the user invokes `/bridge-swift` as the next step (the same way `/verify-ui` follows feature creation). Full mechanics: [`bridge-swift/SKILL.md`](../../bridge-swift/SKILL.md).
...
- [ ] iOS Swift needed? → `iosMain` interface + stub + `/bridge-swift` follow-up emitted
