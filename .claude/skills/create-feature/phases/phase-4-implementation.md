...
¹ A pure display view with no device data (e.g. a static WebView) may need no provider — then `platform` is skipped and `ui-layer` writes only the `expect/actual` composable.

**Division of labor on the platform path** (no overlap):
- **platform** (provider-only): `commonMain` DataSource interface + per-platform `actual` classes (android/ios/**desktop**) + `expect/actual val platformModule`. Writes **no** `@Composable`.
- **ui-layer**: the `expect @Composable PlatformX` + `AndroidView`/`UIKitView`/desktop-fallback actuals under `components/`, plus the normal ViewModel/UiModel/Screen. Loads [architecture/platform.md](../architecture/platform.md) → "Pattern C".
- **integrator**: pulls `platformModule` into `{featurename}Module` via `includes(platformModule)`, lists `{featurename}Module` in `initKoin`'s `modules(...)`, and wires any `androidContext()` the Android actual needs.

**Module-scaffold owner (CRITICAL — build breaks if no one does it)**: `feature/{featurename}/build.gradle.kts` + the module dir structure are normally created by `data-layer`. When `data-layer` is **skipped** (pure `platform-capability` / `native-view`), the **first agent in the set** scaffolds the module instead, from [build-gradle-template.md](../architecture/build-gradle-template.md):

| Profile | Module-scaffold owner |
|---------|------------------------|
| `network`, `mixed` | `data-layer` (unchanged) |
| `platform-capability`, `native-view` **with** a capability | `platform` |
| `native-view` **without** a capability (no provider) | `ui-layer` |
...
1. **Before UI agent**: Read the blueprint's **Pre-Implementation Contract** → extract XTheme missing roles **and the Typography Updates Required** (font swap + type-scale role overrides). Also read the manifests at:
   - `.claude/docs/{featurename}/designs/extracted/icons.json` (Material Symbols, from `/design-ui` sub-step 5)
   - `.claude/docs/{featurename}/designs/extracted/images.json` (`<img>` assets, from `/design-ui` sub-step 6)
   - `.claude/docs/{featurename}/designs/extracted/fonts.json` (design typeface, from `/design-ui` sub-step 6b)
...
2. **Materialize Material Symbols XML drawables** by running the shared downloader **without** `--manifest-only` so it actually downloads, applies the JetBrains-required KMP cleanup pass, extends `DesignSystemResources.kt` for any chrome additions, and inline-migrates any stale `feature/X/drawable/{ident}.xml` plus their Kotlin imports for icons that just promoted from domain to chrome:
   ...
   - **Pass `--html` for the same selected states** the manifest covers (`needsLoading`/`needsFailed`/`needsEmpty` flags from `stitch-project.json.features[{featurename}].states`) **plus one `--html .../stitch_{role}.html` per `secondaryScreens[]` entry** — secondary screens may introduce their own icons, which `/design-ui` already folded into this same `icons.json` at design time.
   - **Why before XTheme**: icons are a deterministic script with no LLM reasoning; running it first surfaces network/404 failures early before any UI work begins. If any download fails, the script reports `http-404` etc. in its summary — fix the failing icon name in the design, re-run `/design-ui` to refresh the manifest, then retry.
   - **What the script does in full mode** (matches the manifest's predictions):
     ...
     - For promoted icons (those whose `users` set includes another feature): deletes the stale `feature/{other}/.../drawable/{ident}.xml` AND rewrites every `.kt` under `feature/{other}/src/` from `Res.drawable.{ident}` → `DesignSystemResources.drawable.{ident}` (imports fixed). Doc-artifact promotion of other features' manifests was already done by `/design-ui`; this step just brings their source in sync.
...
6b. **Secondary screens (only when `features[{featurename}].secondaryScreens[]` is non-empty)**: also pass each secondary screen's screenshot (`.claude/docs/{featurename}/designs/{featurename}_{role}.png`) and instruct the UI agent to implement the blueprint's **`## Component Tree → Secondary Screens`** subsections per their `kind` (see [`_shared/patterns.md` → "Secondary Screens within a Feature"](../../_shared/patterns.md)):
   ...
   - Its strings are already in the blueprint's String Inventory; its icons/images are already in the same manifests (materialized above).
...
- Secondary screens: if `features[{featurename}].secondaryScreens[]` is non-empty, include each `.claude/docs/{featurename}/designs/{featurename}_{role}.png` and implement the blueprint's `Component Tree → Secondary Screens` subsections per `kind` — **`surface`** → host X-component (`XModalBottomSheet`/`XDialog`/…) from `{Feature}ScreenRoot` + `{Feature}UiModel` visibility field + callbacks (Rule 10), no Route; **`screen`** → own `{Feature}{Role}Screen.kt` (own allowlist) + `{Feature}{Role}Route` + `composable<…>` inside the same `NavGraphBuilder.{featurename}()` extension, navigated via a hoisted callback. Child route, **not** a new Integration Point. See `_shared/patterns.md` → "Secondary Screens within a Feature".
...
- Native-view (Rule 14, tag = `native-view`/`mixed`): write the `expect @Composable PlatformX` + `AndroidView`/`UIKitView`/desktop-fallback actuals under `components/`; `{Feature}Content` calls it and stays pure Compose. Load architecture/platform.md → "Pattern C". Consume the DataSource interface from platform (do NOT write the provider).
...
```
1. data-layer (network / mixed only) with:
   - Feature name: {featurename}
...
2. ui-layer with:
   ...
   - Design-aware context (if applicable):
     ...
     - Secondary screens: if `features[{featurename}].secondaryScreens[]` is non-empty, include each `{featurename}_{role}.png` and implement the blueprint's `Component Tree → Secondary Screens` subsections per `kind` (`surface` → host X-component + `{Feature}UiModel` visibility field + callbacks, no Route; `screen` → own `{Feature}{Role}Screen.kt` + `{Feature}{Role}Route` + `composable<…>` in the same `NavGraphBuilder.{featurename}()` extension + hoisted-callback nav). Child route, not a new Integration Point. See `_shared/patterns.md` → "Secondary Screens within a Feature".
   - Localization (Rule 12): create `composeResources/values/strings.xml`; ALL display text via `stringResource(Res.string.*)` — no hardcoded literals. If a blueprint is present, use its String Inventory keys.
   ...
   - Native-view (Rule 14, tag = `native-view`/`mixed`): write the `expect @Composable PlatformX` + `AndroidView`/`UIKitView`/desktop-fallback actuals under `components/`; consume platform's DataSource interface; load architecture/platform.md → "Pattern C".

3. platform (tag ≠ `network`) with:
...
If `platform` (or `ui-layer` for a native view) flagged that an **iOS `actual` needs Swift**, the Kotlin side is complete but the iOS implementation is a stub. Skills never call each other — **surface a route to the user** as part of the completion report:

```
> iOS note — {Feature}'s iOS actual needs a Swift implementation.
> Run `/bridge-swift` for {Feature}Bridge to complete the iOS side.
```

Do **not** invoke `/bridge-swift` yourself. Android + desktop builds pass without it; the iOS framework links once the user completes the bridge. If no Swift dependency was flagged, omit this step entirely.
