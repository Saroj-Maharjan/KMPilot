...
**If (c) finds no previews**: Surface the warning but do NOT block cleanup — previews are required but a missing preview does not break the build or architecture. The user may choose to add them via `/modify-feature`.
