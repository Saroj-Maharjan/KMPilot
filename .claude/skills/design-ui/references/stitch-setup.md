...
How to install and configure the Stitch MCP server for the `/design-ui` skill when it is not yet available in Claude Code.
...
The user must fully restart Claude Code (quit and reopen) so the new MCP server loads. Then re-invoke `/design-ui` (with or without a feature name, depending on where they were in the flow).
...
After it finishes the wizard writes the config to `$HOME/.claude.json` (or `./.mcp.json` if the user picked project scope). The user must restart Claude Code and re-invoke `/design-ui`.
...
After the user restarts Claude Code and re-invokes `/design-ui`, the skill's preflight call to `mcp__stitch__list_projects` should succeed. If it still fails:
...
```
Stitch MCP is not configured. The /design-ui skill needs it for all Stitch operations.
...
Fastest setup (≈2 minutes):
  ...
  4. Re-invoke /design-ui

Full setup guide (OAuth, troubleshooting, alternatives):
  .claude/skills/design-ui/references/stitch-setup.md
