...
2. Map structural Tailwind classes to Compose containers and arrangements (the script does not
   ...
   - **fixed/sticky TOP header → `XScreen` topBar slot; DROP the body's compensating `pt-{header-height}`** (the Stitch HTML adds top-padding to the body to clear the fixed header — in Compose this is handled by `XScreen` stacking `topBar + content`, so the padding is zero-counted in `contentPadding.top`). Tighter: if a real `XScreen` topBar exists, body `contentPadding.top` for header clearance = 0 (add only the design's own first-element padding, typically 8–16 dp). Do NOT carry the header height value into `contentPadding`.
   - **CSS `position:fixed`/`position:sticky` status-bar inset vs. fixed-header offset**: a body `pt-{N}` can mean (a) status-bar safe-area — already handled by the app-shell Scaffold (Rule 12 below); or (b) fixed-header clearance — eliminated by `XScreen` topBar. Both → body `contentPadding.top = 8dp` (design's own first-element offset, not the header height).
   ...
   - **CSS `<div>` container wrapper → `Column` (or `Box` with a SINGLE child). `Box` z-stacks ALL children on top of each other — a background-bearing wrapper `<div>` that contains vertically stacked content MUST become a `Column`, not a `Box`.** A composable that emits ≥2 top-level siblings (a fragment composable) must own its wrapping `Column`/`Row`; it must NEVER rely on the caller's layout context. Reviewer grep: `Box {` containing multi-sibling composable calls where the content is not explicitly overlaid (z-stacking is the intent for overlays only — hero image + overlay gradient + text column). If unsure: any content that should scroll or stack vertically → `Column`.
...
| Material Symbols `<span class="material-symbols-*" data-icon="{name}">` | `XIcon(painter = painterResource({res_reference}))` — look up `{res_reference}` in `extracted/icons.json` (chrome icons → `DesignSystemResources.drawable.{name}`; domain icons → `Res.drawable.{name}`). The manifest is authoritative. Chrome vs domain is decided declaratively by cross-feature usage count; the XML files themselves are materialized by `/create-feature` or `/modify-feature` in design-aware mode (not by `/design-ui`). Filled variants (HTML has `data-weight="fill"`) get a `_fill` suffix in the resource name. **Never fall back to `Icons.Default.*` from the deprecated `material-icons-extended` library.** |
| `position: fixed` / `sticky` bottom | `XScreen` `bottomBar` slot (Rule 13 — never a Scaffold bottomBar in a feature) |
| `position: fixed` / `sticky` top header | `XScreen` `topBar` slot; **DROP** the body's compensating `pt-{header-height}` — the Stitch body offset double-counts against Compose's layout stacking. Body `contentPadding.top` = design's own first-element offset only (usually 8–16 dp, not the header height). |
| Background-bearing `<div>` wrapping vertically stacked content | `Column` (NOT `Box` — `Box` z-stacks all children). A composable that emits ≥2 siblings must own its `Column`/`Row` — never fragment and rely on the caller's container. |
