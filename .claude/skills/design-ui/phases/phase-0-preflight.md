...
2. Call `mcp__stitch__get_project` with `name` set to `stitch-project.json.projectName` to verify the shared project is still accessible.
   - If 404 or project not found:
     ```
     The shared Stitch project ({projectId}) no longer exists. Re-run /design-ui without
...
**Priority order:**
1. **$ARGUMENTS** — If the user invoked `/design-ui productdetail`, use `$ARGUMENTS` directly as the feature name
