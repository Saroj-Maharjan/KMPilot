...
**If fails** (tool not found or connection error): Stitch MCP is not configured. Walk the user through setup using the procedure below — the full reference lives at `.claude/skills/design-ui/references/stitch-setup.md`.
...
3. **If API Key**:
   ...
   - Once they confirm they have the key, tell them to run (replacing `YOUR_API_KEY`):
     ...
     claude mcp add stitch \
       ...
       -s user
     ```
   - Tell them to **fully quit and reopen Claude Code**, then re-invoke `/design-ui`.
...
5. **If "I'll set it up myself"**: point them to `.claude/skills/design-ui/references/stitch-setup.md` for the full guide and stop.
...
> **Next step —** run `/design-ui {featurename}` to design any feature's screens.
```
