...
4d. **Shared state screen code implementation** — rewrites `AppLoadingState.kt` / `AppErrorState.kt` in `core/designsystem/.../app/` to match the approved Stitch design. Gated by `codeImplemented == false`; skip entirely if `codeImplemented == true` (already implemented) or if this feature opted out of a state (`states.{state} == false`).

   For each state in `[loading, failed]` where `features[{featurename}].states.{state} == true` AND `sharedStateScreens.{state}.codeImplemented == false`:
