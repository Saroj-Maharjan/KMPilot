...
1d. **Shared state screen code implementation** — rewrites `AppLoadingState.kt` / `AppErrorState.kt` in `core/designsystem/.../app/` to match the approved Stitch design. Gated by `codeImplemented == false`; skip entirely if `codeImplemented == true` (already implemented) or if this feature opted out of a state (`states.{state} == false`).

   For each state in `[loading, failed]` where `stitch-project.json.features[{featurename}].states.{state} == true` AND `sharedStateScreens.{state}.codeImplemented == false`:
   ...
   4. Set `stitch-project.json.sharedStateScreens.{state}.codeImplemented = true`. Update top-level `updatedAt`. Write the file.

   Verify build: `./gradlew :core:designsystem:assembleAndroidMain`
