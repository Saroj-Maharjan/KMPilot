# `/design-ui` Flow Chart

End-to-end visual reference for what happens when a user invokes `/design-ui {featurename}`. Mermaid diagrams render in GitHub, VS Code, and most markdown viewers. The textual breakdown below each diagram walks through the same flow step by step.
...
```mermaid
flowchart TD
    Start([User: /design-ui featurename]):::user --> InitGate{stitch-project.json exists<br/>AND initState.completedAt set?}
    ...
    S119 --> Done([Complete — user invokes<br/>/create-feature or /modify-feature]):::done
...
| Implementation by `/create-feature` / `/modify-feature` | Feature code must still satisfy Rule 4 (handle all UI states) — generic fallback used; no design reference passed to the UI agent |

---

## 8. Worked example — `/design-ui carsdashboard`
...
For a **second** feature (e.g. `/design-ui transactions`) that also picks Loading + Failed: Step 1.8 sees the shared screens already exist (created during carsdashboard) and is a **no-op**. The transactions feature proceeds directly to its own success-screen design.

---

## 9. Cross-references

- Skill entry: [`design-ui/SKILL.md`](../../../skills/design-ui/SKILL.md)
- Phase Init (project bootstrap + On-Demand Procedures for shared screens): [`design-ui/phases/phase-init.md`](../../../skills/design-ui/phases/phase-init.md)
- Per-feature preflight: [`design-ui/phases/phase-0-preflight.md`](../../../skills/design-ui/phases/phase-0-preflight.md)
- Design phase (Steps 1.1 → 1.19, all gating logic): [`design-ui/phases/phase-1-design.md`](../../../skills/design-ui/phases/phase-1-design.md)
- Blueprint template + skipped-state markers: [`design-ui/references/blueprint-spec.md`](../../../skills/design-ui/references/blueprint-spec.md)
- Stitch MCP usage patterns + known issues: [`design-ui/references/stitch-guide.md`](../../../skills/design-ui/references/stitch-guide.md)
