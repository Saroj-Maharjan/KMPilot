...
|---------|------|-------|--------|---------|
| home | ✅ | ✅ 7 files | ✅ | - |
| profile | ✅ | ⚠️ 2 files | ❌ | /review-feature profile |
...
> **Next step —** run `/clear` to free the context window, then `/test-feature {feature}` to generate tests for one of the flagged features.
```
...
> **Next step —** run `/clear` to free the context window, then `/review-feature {feature}` to validate one of the flagged features.
```
